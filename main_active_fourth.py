...
def train_cnn(j=0):
    ...
    for metric in ["ep"]:  #"bvsb",, "bmax", "bavg"
        cnn = cu.create_cnn(3)
        ...
        for i in range(0, 3):
            ...
            checkpointer = ModelCheckpoint(
                filepath="models_active/fourth/" + metric + "/cnn_active_" + str(i + 1) + "_fourth_" + str(j) + "_best.hdf5",
            ...
            cnn = load_model("models_active/fourth/" + metric + "/cnn_active_" + str(i + 1) + "_fourth_" + str(j) + "_best.hdf5")
            loss, acc = cnn.evaluate(x_valid, y_valid, verbose=1)

            with open('models_active/fourth/active_' + metric + '_results_' + str(j) + '_best.txt', 'a') as f:
