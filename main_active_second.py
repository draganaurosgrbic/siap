...
def train_cnn(j=0):
    ...
    temp = []
    temp_labels = []
    for img_name in os.listdir("train/basset/"):
        img_path = os.path.join("train/basset/", img_name)
        img = cu.load_image_color(img_path)
        img = cv2.resize(img, cu.best_size, interpolation=cv2.INTER_CUBIC)
        temp.append(img)
    ...
    temp = []
    temp_labels = []
    for img_name in os.listdir("train/leonberg/"):
        img_path = os.path.join("train/leonberg/", img_name)
        img = cu.load_image_color(img_path)
        img = cv2.resize(img, cu.best_size, interpolation=cv2.INTER_CUBIC)
        temp.append(img)
        temp_labels.append(0)
    ...
    temp = []
    temp_labels = []
    for img_name in os.listdir("train/samoyed/"):
        img_path = os.path.join("train/samoyed/", img_name)
        img = cu.load_image_color(img_path)
        img = cv2.resize(img, cu.best_size, interpolation=cv2.INTER_CUBIC)
        temp.append(img)
        labels.append(0)
    ...
    for metric in ["bmax"]:  #"bvsb", "ep", "bmax", "bavg"
        cnn = cu.create_cnn(2)
        ...
        for i in range(0, 3):
            ...
            checkpointer = ModelCheckpoint(
                filepath="models_active/second/" + metric + "/cnn_active_" + str(i + 1) + "_second_" + str(j) + "_best.hdf5",
            ...
            cnn = load_model("models_active/second/" + metric + "/cnn_active_" + str(i + 1) + "_second_" + str(j) + "_best.hdf5")
            loss, acc = cnn.evaluate(x_valid, y_valid, verbose=1)

            with open('models_active/second/active_' + metric + '_results_' + str(j) + '_best.txt', 'a') as f:
            ...
            x_total = np.concatenate([x_total, x_remaining[0:(sizes[i] * 6)]])
            y_total = np.concatenate([y_total, y_remaining[0:(sizes[i] * 6)]])

            # izbacimo najboljih n iz ostatka
            x_remaining = x_remaining[(sizes[i] * 6):]
            y_remaining = y_remaining[(sizes[i] * 6):]
