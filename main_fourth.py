...
labele = ["samoyed", "leonberg", "basset"]
img_w = 0
img_h = 0
...
def train_cnn():
    ...
    checkpointer = ModelCheckpoint(filepath="cnn_best_fourth.hdf5", verbose=1, save_best_only=True)

    cnn.fit(x_train, y_train, validation_data=(x_valid, y_valid), epochs=30, batch_size=32, callbacks=[checkpointer],
            verbose=1, shuffle=True)
    cnn.save("cnn_fourth.hdf5")


if __name__ == '__main__':
    # train_cnn()
    # exit(0)
    clf_cnn = load_model("models/cnn_fourth_0038_1000.hdf5")
    ...
    for img_name in os.listdir("test2"):
        ...
        cv2.imwrite('output/cnn/combo/whole/fourth/' + img_name.split('.')[0] + '.png', itest)
