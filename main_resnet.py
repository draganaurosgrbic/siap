        ...
        for y in range(0, img_h, step_size_h):
            for x in range(0, img_w, step_size_w):
                ...
                if roi.shape[0:2] == (size_y, size_x):
                    ...
                    prediction = is_dog(roi)
                    if prediction[0] > min_score and prediction[1]:
...
def process_image(image, rects, min_score=0.9):
    best_rects = []
    best_scores = []
    best_classes = []
...
def train_cnn():
    ...
    temp = []
    temp_labels = []
    for img_name in os.listdir("dogs/train/siamese/"):
        img_path = os.path.join("dogs/train/siamese/", img_name)
    ...
    temp = []
    temp_labels = []
    for img_name in os.listdir("dogs/train/mau/"):
        img_path = os.path.join("dogs/train/mau/", img_name)
        img = load_image_color(img_path)
        img = cv2.resize(img, best_size, interpolation=cv2.INTER_CUBIC)
        temp.append(img)
    ...
    temp = []
    temp_labels = []
    for img_name in os.listdir("dogs/train/persian/"):
        img_path = os.path.join("dogs/train/persian/", img_name)
        img = load_image_color(img_path)
        img = cv2.resize(img, best_size, interpolation=cv2.INTER_CUBIC)
        imgs.append(img)
        labels.append(4)
    ...
    temp = []
    temp_labels = []
    for img_name in os.listdir("dogs/train/basset/"):
        img_path = os.path.join("dogs/train/basset/", img_name)
        img = load_image_color(img_path)
        img = cv2.resize(img, best_size, interpolation=cv2.INTER_CUBIC)
        imgs.append(img)
        labels.append(3)
    ...
    cnn.fit(x_train, y_train, validation_data=(x_valid, y_valid), epochs=10, batch_size=32, verbose=1, shuffle=True)
    cnn.save("cnn.hdf5")


if __name__ == '__main__':
    clf_cnn = load_model("cnn_1242_9658.hdf5")

    for mode in ['fast', 'quality', 'window']:
        for img_name in os.listdir("dogs/test"):
            img_path = os.path.join("dogs/test/", img_name)
            itest = load_image_color(img_path)
            ...
            for startX, startY, endX, endY in windows:
                roi = itest[startY:endY, startX:endX]
                score, class_idx = process_whole_image(roi)
                cv2.putText(itest, labele[class_idx - 1] + "{:6.3f}".format(score),
                            (startX + 5, startY + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                            (36, 255, 12), 2)
                cv2.rectangle(itest, (startX, startY), (endX, endY), (0, 255, 0), 2)
                i += 1

            cv2.imwrite('output/resnet/' + mode + '/' + img_name.split('.')[0] + '.png', itest)
